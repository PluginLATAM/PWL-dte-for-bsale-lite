...
class Settings extends BasePage
{
	...
	public function enqueue_scripts(string $hook): void
	{
		...
		wp_register_script('pwl-dte-for-bsale-settings', false, ['jquery'], PWL_DTE_VERSION, true);
		...
		(function($){
			$('#pwl-dte-for-bsale-load-offices').on('click',function(){
				var $btn=$(this),$status=$('#pwl-dte-for-bsale-offices-status');
				$btn.prop('disabled',true);
				$status.text(pwlDteSettings.labels.loading);
				$.ajax({
					url:ajaxurl,method:'POST',
					data:{action:'pwl_dte_get_offices',nonce:pwlDteSettings.nonce},
					success:function(r){
						if(!r.success){
							$status.text(pwlDteSettings.labels.errorPrefix+(r.data&&r.data.message?r.data.message:pwlDteSettings.labels.unknownError));
							$btn.prop('disabled',false);return;
						}
						$('.pwl-dte-for-bsale-office-select').each(function(){
							var $s=$(this),cur=$s.data('current')||$s.val(),first=$s.find('option:first').clone();
							$s.empty().append(first);
							$.each(r.data.offices,function(i,o){
								var $o=$('<option>').val(o.id).text(o.name+' (#'+o.id+')');
								if(String(o.id)===String(cur))$o.prop('selected',true);
								$s.append($o);
							});
						});
		...
		wp_add_inline_script('pwl-dte-for-bsale-settings', $js);
		wp_enqueue_script('pwl-dte-for-bsale-settings');
	}
	...
	protected function render_content(): void
	{
		...
		<?php echo $this->get_notices_html(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		...
		<form method="post" action="options.php">
			...
			<div style="margin-top:24px;">
				<?php echo Components::button(__("Guardar configuración", "pwl-dte-for-bsale"), "primary", ["type" => "submit"]); ?>
			</div>
		</form>
		<?php
	}
	...
	private function render_offices_tab(): void
	{
		...
		<div style="margin-bottom:16px;">
			<?php echo Components::button(__("Cargar sucursales desde Bsale", "pwl-dte-for-bsale"), "secondary", ["attrs" => ["id" => "pwl-dte-for-bsale-load-offices"]]); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					...
					<select id="pwl_dte_stock_office_id" name="pwl_dte_stock_office_id" class="pwl-dte-for-bsale-office-select">
						...
						<?php if (!empty($stock_office)): ?>
							<option value="<?php echo esc_attr($stock_office); ?>" selected>
								<?php printf(esc_html__("Sucursal #%s", "pwl-dte-for-bsale"), esc_html($stock_office)); /* translators: %s: office ID number */ ?>
							</option>
						<?php endif; ?>
					</select>
					...
					>
						...
						<?php if (!empty($map[$method_id])): ?>
							<option value="<?php echo esc_attr($map[$method_id]); ?>" selected>
								<?php printf(esc_html__("Sucursal #%s", "pwl-dte-for-bsale"), esc_html($map[$method_id])); /* translators: %s: office ID number */ ?>
							</option>
						<?php endif; ?>
					</select>
				</td>
