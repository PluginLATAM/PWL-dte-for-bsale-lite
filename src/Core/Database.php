...
class Database
{
	...
	public function get_webhook_events(
		...
		global $wpdb;
		$where = $this->webhook_where_clause($topic, $status);
		$sql   = $where
			? $wpdb->prepare(
				"SELECT * FROM {$this->webhook_table} WHERE {$where['sql']} ORDER BY received_at DESC LIMIT %d OFFSET %d",
				...[...$where['values'], $limit, $offset],
			)
			: $wpdb->prepare(
				"SELECT * FROM {$this->webhook_table} ORDER BY received_at DESC LIMIT %d OFFSET %d",
				$limit, $offset,
			);
		return $wpdb->get_results($sql, ARRAY_A) ?: []; // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	}
	...
	public function count_webhook_events(string $topic = '', string $status = ''): int
	{
		...
		$where = $this->webhook_where_clause($topic, $status);
		return $where
			? (int) $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$this->webhook_table} WHERE {$where['sql']}", ...$where['values'])) // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			: (int) $wpdb->get_var("SELECT COUNT(*) FROM {$this->webhook_table}"); // phpcs:ignore WordPress.DB.DirectDatabaseQuery
	}

	public function delete_webhook_events(): bool
	{
		global $wpdb;
		return false !== $wpdb->query("TRUNCATE TABLE {$this->webhook_table}"); // phpcs:ignore WordPress.DB.DirectDatabaseQuery
	}

	// ── Internals ─────────────────────────────────────────────────────────────

	/** @return array{sql: string, values: list<string>}|null */
	private function webhook_where_clause(string $topic, string $status): ?array
	{
		$parts  = [];
		$values = [];
		...
		return $parts ? ['sql' => implode(' AND ', $parts), 'values' => $values] : null;
	}
	...
	{
		global $wpdb;
		$count = (int) $wpdb->get_var("SELECT COUNT(*) FROM {$this->webhook_table}"); // phpcs:ignore WordPress.DB.DirectDatabaseQuery
		if ($count <= $max) {
			return;
		}
		$wpdb->query( // phpcs:ignore WordPress.DB.DirectDatabaseQuery
			$wpdb->prepare(
				"DELETE FROM {$this->webhook_table} WHERE id NOT IN (
					SELECT id FROM (
						SELECT id FROM {$this->webhook_table} ORDER BY received_at DESC LIMIT %d
					) AS keep
				)",
				$max,
			),
